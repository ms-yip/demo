<template>
  <h3 style="margin-left:10px">日志信息</h3>
  <div class="main">
    <div class="container">
      <div class="tools">
...
import Tools from '@/views/index/graphTools'
import RightDrawer from '@/views/index/RightDrawer.vue';
import { History } from "@antv/x6-plugin-history";
import { Export } from "@antv/x6-plugin-export";
import { Snapline } from "@antv/x6-plugin-snapline";
import { Stencil } from "@antv/x6-plugin-stencil";
import { createPorts } from '@/views/flow/shape/shape';
import { onMounted, ref, nextTick, reactive, getCurrentInstance, watch  } from "vue";
...
import { log } from '@/views/index/editor/log';
...
function nodeAddEvent() {
  ...
  _graph.value.on('node:click', (target) => {
    ...
    const from = parseInt(Math.random() * 10)
    monacoEditor.setValue(log.split('\n').slice(from, from + parseInt(Math.random() * 60)).join('\n'))

    // monacoEditor.setValue(JSON.parse(JSON.stringify(log).split('\n\tat').slice(x, x + 100).join('\n\tat')));
  })
  // 连线绑定悬浮事件
